...
const QUICK_PROMPTS = [
  { label: 'CHECK AVAILABILITY', message: 'Is there any vacant table today?' },
  { label: 'MENU RECOMMENDATION', message: 'What are your recommended menu items?' },
  { label: 'RESERVATION FAQ', message: 'How do I make a table reservation?' },
];
  ...
  const [messages, setMessages] = useState<DisplayMessage[]>([
    {
      id: 'greeting',
      role: 'assistant',
      content: 'Hello! Welcome to Megatha. How may I assist you with table reservations or menu recommendations today?',
    },
  ]);
      ...
      {isOpen && (
        <>
          <div 
            ...
            className="fixed inset-0 bg-black/40 backdrop-blur-md md:hidden z-40 animate-in fade-in duration-200"
          />


          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] h-[75vh] md:absolute md:top-auto md:left-auto md:translate-x-0 md:translate-y-0 md:bottom-0 md:right-0 md:w-[400px] md:h-[550px] bg-white shadow-2xl flex flex-col rounded-none overflow-hidden animate-in fade-in zoom-in-95 duration-300 z-50 border border-black/10">

            <div className="bg-black text-white px-[5vw] py-[4vw] md:px-[1.2vw] md:py-[1vw] flex items-center justify-between border-b border-white/5 shrink-0">
            ...
            <div className="flex-1 overflow-y-auto bg-canvas p-[5vw] md:p-[1.2vw] flex flex-col gap-[4vw] md:gap-[1vw]">
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`max-w-[85%] px-[4vw] py-[3vw] md:px-[1vw] md:py-[0.8vw] border text-[3.5vw] md:text-[0.85vw] font-medium tracking-wide leading-[1.4] whitespace-pre-wrap ${
                      msg.role === 'user'
                        ? 'bg-[#6E3A2F] border-[#6E3A2F] text-white rounded-none text-right'
                  ...
                  {msg.suggestedTables && msg.suggestedTables.length > 0 && (
                    <div className="flex flex-wrap gap-[2vw] md:gap-[0.5vw] mt-[2vw] md:mt-[0.6vw] max-w-[85%] justify-start">
                      {msg.suggestedTables.map((table) => (
                        <div
                          key={table.id}
                          className="border border-[#6E3A2F]/30 bg-[#6E3A2F]/5 text-[#6E3A2F] text-[2.6vw] md:text-[0.62vw] font-bold tracking-wider px-[3vw] py-[1.5vw] md:px-[0.7vw] md:py-[0.35vw]"
                        >
                          {table.name} · {table.area} · {table.capacity} pax
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {mutation.isPending && (
                <div className="w-full flex justify-start animate-pulse">
                  <div className="bg-white border border-black/5 px-[4vw] py-[3vw] md:px-[1vw] md:py-[0.8vw] flex items-center gap-[1vw] md:gap-[0.3vw]">
                    <span className="text-[2.8vw] md:text-[0.7vw] font-bold text-black/40  tracking-widest">
                      AI IS TYPING
                    </span>
            ...
            <div className="bg-canvas border-t border-black/5 px-[5vw] pt-[3vw] pb-[1vw] md:px-[1.2vw] md:pt-[0.8vw] md:pb-[0.2vw] flex flex-wrap gap-[2vw] md:gap-[0.5vw] shrink-0">
              {QUICK_PROMPTS.map((prompt) => (
                <button
                  key={prompt.label}
                  onClick={() => handleSend(prompt.message)}
                  disabled={mutation.isPending}
                  className="bg-white border border-black/10 hover:border-black text-black text-[2.8vw] md:text-[0.65vw] font-bold tracking-wider px-[3vw] py-[1.5vw] md:px-[0.8vw] md:py-[0.4vw] transition-all duration-200 active:bg-black active:text-white disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {prompt.label}
                </button>
              ))}
            ...
            <div className="p-[5vw] md:p-[1.2vw] bg-white border-t border-black/5 flex items-center gap-[3vw] md:gap-[0.8vw] shrink-0">
              <input
                type="text"
                value={input}
