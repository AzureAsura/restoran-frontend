...
import POSBoard from '@/components/admin/pos/POSBoard';
import POSSkeleton from '@/components/admin/pos/POSSkeleton';

const POSPage = async () => {
  const queryClient = getQueryClient();
  const cookieHeader = (await cookies()).toString();

  // fetchQuery (not prefetchQuery) — Tables endpoint ber-auth, lihat catatan
  // sama di app/(admin)/admin/tables/page.tsx. Menu tetap public, tapi
  // di-fetchQuery juga di sini biar bisa Promise.all bareng tanpa error salah
  // satu ke-swallow diam-diam.
  try {
    await Promise.all([
      queryClient.fetchQuery(tablesQueryOptions(cookieHeader)),
      queryClient.fetchQuery(menuQueryOptions()),
    ]);
  } catch (error) {
    const message = error instanceof ApiError ? error.message : 'Failed to load POS data.';
