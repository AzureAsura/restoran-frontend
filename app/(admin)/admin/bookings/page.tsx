...
import BookingsBoard from '@/components/admin/bookings/BookingsBoard';
import BookingsSkeleton from '@/components/admin/bookings/BookingsSkeleton';
...
const BookingsPage = async () => {
  ...
  try {
    await queryClient.fetchQuery(bookingsQueryOptions({ date: today }, cookieHeader));
  } catch (error) {
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <Suspense fallback={<BookingsSkeleton />}>
        <BookingsBoard initialDate={today} />
      </Suspense>
    </HydrationBoundary>
  );
};
